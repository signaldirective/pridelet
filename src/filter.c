...
static void filter_pride(context_t *);
...
const lookup[] =
{
    ...
    { "pride", filter_pride, "apply a pride flag colour effect" },
};
...
static void filter_pride(context_t *cx)
{
    unsigned int x, y, w, h;

    if(!cx->pride_palette || !cx->pride_ncolors)
        return;
        ...
        for(x = 0; x < w; x++)
    {
        ...
        if(ch != (unsigned char)' ')
        {
            caca_set_color_argb(cx->torender,
                cx->pride_palette[(x / 2 + y + cx->lines) % cx->pride_ncolors],
                0x0000);
            caca_put_char(cx->torender, x, y, ch);
        }
    }
